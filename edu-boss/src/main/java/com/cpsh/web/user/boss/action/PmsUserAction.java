package com.cpsh.web.user.boss.action;
...
import com.cpsh.common.page.PageBean;
import com.cpsh.facade.user.Inf.PmsUserFacade;
import com.cpsh.facade.user.entity.PmsUser;
import com.cpsh.facade.user.enums.UserStatusEnum;
import com.cpsh.facade.user.enums.UserTypeEnum;
import com.cpsh.web.user.boss.base.BaseAction;
...
public class PmsUserAction extends BaseAction {
    ...
    public String editPmsUserUI() {
        try {
            ...
            // 普通用户没有修改超级管理员的权限
            if (UserTypeEnum.ADMIN.getValue().equals(
                    this.getLoginedUser().getUserType())
                    && UserTypeEnum.ADMIN.getValue().equals(
                            pmsUser.getUserType())) {
    ...
    public String resetUserPwd() {
        try {
            ...
            pmsUserFacade
                    .updateUserPwd(userId, DigestUtils.sha1Hex(newPwd), false);
    ...
    public String userChangeOwnPwd() {
        try {
            ...
            // 更新密码
            pmsUserFacade.updateUserPwd(user.getId(), DigestUtils.sha1Hex(newPwd),
                    true);
